...
export default function LogList() {
    ...
    return (
      <div className="text-center py-8 opacity-50">
        <div className="text-4xl mb-2 flex justify-center">
          <DocumentTextIcon className="w-12 h-12 text-text-muted" />
        </div>
        <p className="text-sm font-bold text-text-muted">Belum ada riwayat.</p>
      </div>
    );
  ...
  return (
    <div className="relative pl-2">
      {/* Timeline Line */}
      <div className="absolute left-2 top-2 bottom-2 w-0.5 bg-border-subtle"></div>
        ...
        {logs.map((log) => {
          const getLogStyle = (msg) => {
            ...
            return {
              ...
              color: "bg-text-muted border-border-subtle shadow-border-subtle",
            };
          };
          ...
          return (
            <div key={log.id} className="relative pl-8 group">
              ...
              <div
                className={`absolute left-[-5px] top-0 w-8 h-8 rounded-full border-4 border-bg-canvas z-10 flex items-center justify-center transition-transform group-hover:scale-110 shadow-lg ${style.color}`}
              >
                {style.icon}
              </div>

              <div className="bg-bg-surface p-4 rounded-2xl shadow-sm border border-border-subtle hover:shadow-md transition-all">
                <div className="flex justify-between items-start gap-3 mb-1">
                  <p className="text-sm font-medium text-text-secondary leading-snug group-hover:text-text-main">
                    {log.message}
                  </p>
                  <span className="text-[10px] font-bold text-text-muted bg-bg-subtle px-2 py-1 rounded-lg border border-border-subtle whitespace-nowrap">
